# Modified to facilitate having accurate sheet numbers when putting layers on seperate sheets.
# comment 3 in the titleblock is set to the sheet number
# comment 4 in the titleblock is set to the total number of sheets
...
def plot_gerbers(board, output_path, templates, enabled_templates, del_temp_files, create_svg, del_single_page_files, dialog_panel):
    ...
    # get number of sheets, and set comment 4 (index 3) to total layer count
    totlayernumbercount = len(templates_list)                   # long name to ensure it's not somewhere else too
    boardTitleblock = board.GetTitleBlock()
    boardTitleblock.SetComment(3, totlayernumbercount)          # set total sheet count (comment 4 has index 3)
    currentSheetcountnum = 0                                    # long name to ensure it's not somewhere else too, incremented before plotting each template

    # Iterate over the templates
    for template in templates_list:
        template_name = template[0]   
        currentSheetcountnum = currentSheetcountnum + 1         # increment to keep sheet number accurate     
        ...
        for layer_info in template[3]:
            ...
            try:
                boardTitleblock.SetComment(2, currentSheetcountnum)     # update the sheet number, and increment sheet number before using, comment 3 has index 2
